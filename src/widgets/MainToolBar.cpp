	...
	mRedoAction(redoAction)
{
    ...
    mUploadButton->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
    mUploadButton->addAction(mUploadAction);
    mUploadButton->setDefaultAction(mUploadAction);
    ...
    mUploadAction->setToolTip(tr("Upload Screen Capture to cloud"));
    mUploadAction->setIcon(IconLoader::load(QLatin1Literal("cloud_upload.svg")));
    mUploadAction->setShortcut(Qt::SHIFT + Qt::Key_U);
    mUploadAction->setEnabled(true);
    connect(mUploadAction, &QAction::triggered, this, &MainToolBar::uploadActionTriggered);
...
QAction* MainToolBar::uploadAction() const
{
