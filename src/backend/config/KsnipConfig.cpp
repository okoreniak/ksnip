...
{
    return loadValue(KsnipConfigOptions::uploaderTypeString(), static_cast<int>(UploaderType::Ushare)).value<UploaderType >();
}
...
QString KsnipConfig::ushareUsername() const
{
    auto defaultUsername = QStringLiteral("");
    return loadValue(KsnipConfigOptions::ushareUsernameString(), defaultUsername).toString();
}

void KsnipConfig::setUshareUsername(const QString& username)
{
    if (ushareUsername() == username) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareUsernameString(), username);
}

QByteArray KsnipConfig::ushareClientId() const
{
    auto defaultClientId = QStringLiteral("");
    return loadValue(KsnipConfigOptions::ushareClientIdString(), defaultClientId).toByteArray();
}

void KsnipConfig::setUshareClientId(const QString& clientId)
{
    if (ushareClientId() == clientId) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareClientIdString(), clientId);
}

QByteArray KsnipConfig::ushareClientSecret() const
{
    auto defaultClientSecret = QStringLiteral("");
    return loadValue(KsnipConfigOptions::ushareClientSecretString(), defaultClientSecret).toByteArray();
}

void KsnipConfig::setUshareClientSecret(const QString& clientSecret)
{
    if (ushareClientSecret() == clientSecret) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareClientSecretString(), clientSecret);
}

QByteArray KsnipConfig::ushareAccessToken() const
{
    auto defaultAccessToken = QStringLiteral("");
    return loadValue(KsnipConfigOptions::ushareAccessTokenString(), defaultAccessToken).toByteArray();
}

void KsnipConfig::setUshareAccessToken(const QString& accessToken)
{
    if (ushareAccessToken() == accessToken) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareAccessTokenString(), accessToken);
}

QByteArray KsnipConfig::ushareRefreshToken() const
{
    auto defaultRefreshToken = QStringLiteral("");
    return loadValue(KsnipConfigOptions::ushareRefreshTokenString(), defaultRefreshToken).toByteArray();
}

void KsnipConfig::setUshareRefreshToken(const QString& refreshToken)
{
    if (ushareRefreshToken() == refreshToken) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareRefreshTokenString(), refreshToken);
}

bool KsnipConfig::ushareForceAnonymous() const
{
    return loadValue(KsnipConfigOptions::ushareForceAnonymousString(), false).toBool();
}

void KsnipConfig::setUshareForceAnonymous(bool  enabled)
{
    if (ushareForceAnonymous() == enabled) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareForceAnonymousString(), enabled);
}

bool KsnipConfig::ushareLinkDirectlyToImage() const
{
    return loadValue(KsnipConfigOptions::ushareLinkDirectlyToImageString(), false).toBool();
}

void KsnipConfig::setUshareLinkDirectlyToImage(bool  enabled)
{
    if (ushareLinkDirectlyToImage() == enabled) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareLinkDirectlyToImageString(), enabled);
}

bool KsnipConfig::ushareAlwaysCopyToClipboard() const
{
    return loadValue(KsnipConfigOptions::ushareAlwaysCopyToClipboardString(), true).toBool();
}

void KsnipConfig::setUshareAlwaysCopyToClipboard(bool  enabled)
{
    if (ushareAlwaysCopyToClipboard() == enabled) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareAlwaysCopyToClipboardString(), enabled);
}

bool KsnipConfig::ushareOpenLinkInBrowser() const
{
    return loadValue(KsnipConfigOptions::ushareOpenLinkInBrowserString(), true).toBool();
}

void KsnipConfig::setUshareOpenLinkInBrowser(bool enabled)
{
    if (ushareOpenLinkInBrowser() == enabled) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareOpenLinkInBrowserString(), enabled);
}

QString KsnipConfig::ushareBaseUrl() const
{
    return loadValue(KsnipConfigOptions::ushareBaseUrlString(), DefaultValues::UshareBaseUrl).toString();
}

void KsnipConfig::setUshareBaseUrl(const QString &baseUrl)
{
    if (imgurBaseUrl() == baseUrl) {
        return;
    }
    saveValue(KsnipConfigOptions::ushareBaseUrlString(), baseUrl);
}
