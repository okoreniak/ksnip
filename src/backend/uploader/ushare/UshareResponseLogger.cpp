...
void UshareResponseLogger::writeLogEntry(const QString &logEntry) const
{
...
void UshareResponseLogger::createPathIfRequired() const
{
    QDir qdir;
    qdir.mkpath(mLogPath);
}

QString UshareResponseLogger::getLogEntry(const UshareResponse &response) const
{
    auto separator = QStringLiteral(",");
    auto deleteLink = QStringLiteral("https://Ushare.com/delete/") + response.deleteHash();
...
QStringList UshareResponseLogger::getLogs() const
{
