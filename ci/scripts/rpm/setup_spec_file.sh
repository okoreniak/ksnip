...
cp ci/rpm/phonexa-screens.spec .
...
cat changelog >> phonexa-screens.spec

echo "--> Update version"
sed -i "s/Version: X.X.X/Version: ${VERSION_NUMBER}/" phonexa-screens.spec
sed -i "s;cmake .;cmake . -DVERSION_SUFIX=${VERSION_SUFFIX} -DBUILD_NUMBER=${BUILD_NUMBER} -DCMAKE_INSTALL_PREFIX=${INSTALL_PREFIX} -DCMAKE_BUILD_TYPE=${BUILD_TYPE};" phonexa-screens.spec # ; is the delimiter
